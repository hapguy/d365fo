...
namespace DataPackageHandler
{
    public class PackageExporter
    {
        public static void ExportPackage(Resources d365Client, string filePath)
        {
            ...
            var executionId = d365Client.DataManagementDefinitionGroups.ExportToPackage("Integration_Outbound_Flow_01", Guid.NewGuid().ToString(), string.Empty, false, "UK28").GetValue();
            ...
            try
            {
                ...
                //string fileNameDT = Path.Combine(filePath, Guid.NewGuid().ToString() + ".zip");
                string fileNameDT = "TB_" +
                    DateTime.Now.ToString("yyyyMMdd_hhmmss") + ".zip";
                blob.DownloadToFile(Path.Combine(filePath, fileNameDT), System.IO.FileMode.Create);
                Console.WriteLine("Downloading the file ...Complete");
                Console.WriteLine(String.Format("File saved under {0}", Path.GetFullPath(filePath) + fileNameDT));
            }
            catch
            {
